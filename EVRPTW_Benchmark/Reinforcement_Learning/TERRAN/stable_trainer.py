...
def resolve_stable_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    world_size = training.get("distributed_world_size", 1)
    if isinstance(world_size, bool) or not isinstance(world_size, int) or world_size < 1:
        raise ValueError("training.distributed_world_size must be a positive integer")
    # Do not insert the default key: historical single-GPU checkpoint signatures
    # are validated against their exact stored configuration.
    effective = training["num_envs_per_gpu"] * training["logical_microbatches_per_epoch"] * world_size
    if int(training.get("effective_batch_size", effective)) != effective:
        raise ValueError("effective_batch_size disagrees with physical batch, microbatch count and world size")
    ...
    if not isinstance(training.get("allow_dataset_relocation_resume", False), bool):
        raise ValueError("training.allow_dataset_relocation_resume must be boolean")
    if training.get("allow_dataset_relocation_resume") and not training.get("resume_checkpoint"):
        raise ValueError("allow_dataset_relocation_resume requires a resume checkpoint")
    return cfg
...
def config_signature(cfg: dict[str, Any]) -> str:
    ...
    for key in ("resume_checkpoint", "actor_warm_start", "checkpoint_interval", "debug",
                "allow_batch_resize_resume", "allow_dataset_relocation_resume"):
...
def validate_resume_checkpoint(payload: dict, cfg: dict, *, seed: int,
    ...
    def geometry(config, key):
        return config["training"].get(key, 1 if key == "distributed_world_size" else None)

    changed = {key: {"old": geometry(original, key), "new": geometry(cfg, key)}
               for key in BATCH_RESUME_FIELDS
               if geometry(original, key) != geometry(cfg, key)}
    allowed = bool(cfg["training"].get("allow_batch_resize_resume", False))
    relocation_allowed = bool(cfg["training"].get("allow_dataset_relocation_resume", False))
    path_fields = ("stage2_dataset_path", "stage2_family_root")
    relocated = {key: {"old": original.get("data", {}).get(key), "new": cfg.get("data", {}).get(key)}
                 for key in path_fields if original.get("data", {}).get(key) != cfg.get("data", {}).get(key)}
    if relocation_allowed:
        old_hash = original.get("data", {}).get("training_index_sha256")
        if not old_hash or old_hash != cfg.get("data", {}).get("training_index_sha256"):
            raise ValueError("dataset relocation requires matching nonempty training_index_sha256")
        old_validation_hash = original.get("evaluation", {}).get("validation_index_sha256")
        if old_validation_hash and old_validation_hash != cfg.get("evaluation", {}).get("validation_index_sha256"):
            raise ValueError("dataset relocation requires matching validation_index_sha256")
    if original_signature != signature:
        if not allowed and not relocation_allowed:
        ...
        for comparable in (comparable_old, comparable_new):
            ...
            if relocation_allowed:
                for key in path_fields:
                    comparable.get("data", {}).pop(key, None)
        if config_signature(comparable_old) != config_signature(comparable_new):
            raise ValueError("batch resize resume permits only changes to "
                             + ", ".join(BATCH_RESUME_FIELDS if allowed else ())
                             + ("; dataset relocation permits only " + ", ".join(path_fields)
                                if relocation_allowed else ""))
    return {"source_checkpoint": str(Path(source).resolve()),
            ...
            "allow_dataset_relocation_resume": relocation_allowed,
            "relocated_dataset_paths": relocated,
            "old_batch_geometry": {key: geometry(original, key) for key in BATCH_RESUME_FIELDS},
            "new_batch_geometry": {key: geometry(cfg, key) for key in BATCH_RESUME_FIELDS},
...
def rollout_policy_kl(agent, records) -> float:
    ...
    device = next(agent.parameters()).device
    distributed = DistributedContext.current()
    total, count = 0.0, 0
    with distributed.local_phase("policy KL replay"):
        for batch in records:
            ...
            for step, observation in enumerate(batch.observations):
                ...
                count += int(valid.sum())
    total, count = distributed.sum_values([total, count])
...
def optimize_rollouts(agent, actor_optimizer, critic_optimizer, records, cfg, state):
    ...
    distributed = DistributedContext.current()
    with distributed.local_phase("advantage preparation"):
        raw_advantages = [make_advantages(batch, state) for batch in records]
        count = sum(int(batch.valid.sum()) for batch in records)
        squared = sum(float(adv[batch.valid].double().square().sum())
                      for batch, adv in zip(records, raw_advantages))
        zeros = sum(int((adv[batch.valid] == 0).sum()) for batch, adv in zip(records, raw_advantages))
        trajectory_count = sum(int(batch.valid.any(dim=0).sum()) for batch in records)
    count, squared, zeros, trajectory_count = distributed.sum_values([count, squared, zeros, trajectory_count])
    if count == 0:
    ...
    targets = torch.cat([batch.cost_returns.flatten() for batch in records]) if records else torch.empty(0)
    weights = torch.cat([trajectory_state_weights(batch.valid).flatten() for batch in records]) if records else torch.empty(0)
    agent.critic.popart.update_stats(targets, mask=weights > 0, weights=weights,
                                     optimizer=critic_optimizer, distributed=distributed)
    ...
    metrics = dict(actor_loss=0.0, critic_loss=0.0, entropy=0.0, actor_grad_norm=0.0,
                   ...
                   zero_advantage_fraction=zeros / max(count, 1))
    for update in range(training["ppo_update_epochs"]):
        ...
        with distributed.local_phase("PPO backward"):
            for batch, advantage in zip(records, advantages):
                ...
                for start in range(0, steps, training["ppo_step_chunk_size"]):
                    result = loss_chunk(agent, batch, advantage, cfg, start,
                                        ...
                                        trajectory_denominator=int(trajectory_count),
                    ...
                    for key, value in chunk_diag.items():
                        diag[key] += value
                    del loss, result
        distributed.sum_gradients(actor_parameters + critic_parameters)
        totals = distributed.sum_values(update_losses + list(diag.values()))
        update_losses = totals[:3]
        diag = dict(zip(diag, totals[3:]))
...
def train_stable_cost(config: dict[str, Any], seed: int, device=None) -> Path:
    ...
    distributed = DistributedContext.current()
    if int(training.get("distributed_world_size", 1)) != distributed.world_size:
        raise ValueError("declared distributed_world_size does not match the initialized process group")
    ...
    distributed.main_call(prepare_output)
    with distributed.local_phase("training initialization"):
    ...
    distributed.broadcast_model(agent)
    ...
    def write_contract():
        ...
        atomic_json(output / "training_contract.json", {"schema": SCHEMA, "training_signature": signature,
            ...
            "effective_instances": training["effective_batch_size"],
            "policy_time_unit": training["rollout_steps"], "critic_weighting": "trajectory_mean",
            "gradient_reduction": "sum_with_global_trajectory_denominator",
            "popart_optimizer_state": "reset_output_layer_moments_on_stats_update"})

    distributed.main_call(write_contract)
    # Rank identity only affects sampling; it must never enter a checkpoint's
    # semantic configuration/signature. Every replica uses the same global
    # shuffle cursor and consumes disjoint physical-batch blocks from it.
    env_config = deepcopy(cfg)
    if distributed.world_size > 1:
        env_config["data"].update(stage2_sampling_rank=distributed.rank,
                                  stage2_sampling_world_size=distributed.world_size,
                                  stage2_sampling_batch_size=training["num_envs_per_gpu"])
    with distributed.local_phase("environment initialization"):
        envs, pool = make_envs(env_config, seed)
    started = time.perf_counter()
    progress_path = output / "progress.json"
    metrics_path = output / "metrics.jsonl"
    distributed.main_call(lambda: atomic_json(progress_path,
        {"status": "running", "epoch": state.epoch, "pid": os.getpid(),
         "distributed_world_size": distributed.world_size}))
    ...
    def save():
        distributed.main_call(save_local)
    ...
    try:
        for epoch in range(state.epoch + 1, training["epochs"] + 1):
            ...
            agent.train()
            records = []
            epoch_seed = seed + epoch * 100_000 + distributed.rank * training["logical_microbatches_per_epoch"]
            set_seed(epoch_seed)
            collect_start = time.perf_counter()
            with distributed.local_phase("rollout collection"):
                for microbatch in range(training["logical_microbatches_per_epoch"]):
                    batch = collect_rollout(agent, envs, rollout_steps=training["rollout_steps"],
                        decode_mode="sample", device=device, seed=epoch_seed + microbatch,
            ...
            failure_sum, unserved_sum, terminal_count, cost_sum, effective_instances = distributed.sum_values([
                sum(float(batch.terminal_failure.double().sum()) for batch in records),
                sum(float(batch.unserved_fraction.double().sum()) for batch in records),
                sum(batch.terminal_failure.numel() for batch in records),
                sum(float(batch.cost_returns[0].double().sum()) for batch in records),
                sum(batch.actions.shape[1] for batch in records)])
            failure_rate = failure_sum / max(terminal_count, 1)
            unserved = unserved_sum / max(terminal_count, 1)
            phase_used, lambda_used = state.phase, state.lambda_usd
            distributed.main_call(lambda: atomic_json(progress_path, {
                "status": "running", "pid": os.getpid(), "epoch": epoch,
                "stage": "update", "phase": state.phase, "distributed_world_size": distributed.world_size}))
            ...
            state.epoch = epoch
            with distributed.local_phase("sample cursor"):
                state.sample_count = int(pool.global_committed_cursor if hasattr(pool, "global_committed_cursor")
                                         else pool.sample_count)
                sampled_ids = pool.drain_sampled_view_ids() if hasattr(pool, "drain_sampled_view_ids") else []
            cursors = distributed.gather_objects(state.sample_count)
            if len(set(cursors)) != 1:
                raise RuntimeError(f"distributed sampler cursors disagree: {cursors}")
            sampled_by_rank = distributed.gather_objects(sampled_ids)
            physical_batch = training["num_envs_per_gpu"]
            # Reconstruct the original seeded stream, rather than rank-major ID
            # ordering, so a topology-changing resume can be audited exactly.
            sampled_ids = [view_id for start in range(0, max(map(len, sampled_by_rank)), physical_batch)
                           for rank_ids in sampled_by_rank for view_id in rank_ids[start:start + physical_batch]]
            distributed.main_call(lambda: append_jsonl(output / "sampled_view_ids.jsonl", {
                "epoch": epoch, "start_cursor": state.sample_count - len(sampled_ids),
                "end_cursor": state.sample_count, "view_ids": sampled_ids}))
            valid_transitions = int(distributed.sum_values([sum(int(batch.valid.sum()) for batch in records)])[0])
            ...
            if state.phase == "feasibility":
                ...
                if state.success_streak >= settings["warmup_min_epochs"]:
                    state.phase = "cost"
            epoch_wall_s, collect_wall_s, update_wall_s = distributed.max_values([
                time.perf_counter() - epoch_start, collect_wall_s, update_wall_s])
            ...
            if distributed.world_size > 1:
                timing_rows = distributed.gather_objects(rollout_timings)
                rollout_timings = {key: max(row.get(key, 0.0) for row in timing_rows)
                                   for key in set().union(*(row.keys() for row in timing_rows))}
            row = {"schema": SCHEMA, "epoch": epoch, "phase": phase_used, "next_phase": state.phase,
                   ...
                   "cost_accrued_mean_usd": cost_sum / max(terminal_count, 1), "lambda_usd": lambda_used,
                   "lambda_next_usd": state.lambda_usd, "popart_std_usd": popart_std,
                   "effective_instances": int(effective_instances), "distributed_world_size": distributed.world_size,
            ...
            if str(device).startswith("cuda"):
                allocated, reserved = distributed.max_values([
                    torch.cuda.max_memory_allocated(device) / 2**20,
                    torch.cuda.max_memory_reserved(device) / 2**20])
                row.update(cuda_peak_allocated_mb=allocated, cuda_peak_reserved_mb=reserved)
            ...
            distributed.main_call(write_metrics)
            del records, batch
            eval_cfg = cfg.get("evaluation", {})
            interval = int(eval_cfg.get("eval_interval", 0))
            if interval and epoch % interval == 0:
                save()  # A long rank-zero validation must not hide a completed epoch.

                def evaluate_and_save_best():
                    atomic_json(progress_path, {"status": "running", "pid": os.getpid(), "epoch": epoch,
                        "stage": "validate", "phase": state.phase,
                        "distributed_world_size": distributed.world_size})
                ...
                state = StableState(**distributed.main_call(evaluate_and_save_best))
        ...
        save()
        distributed.main_call(lambda: atomic_json(progress_path,
            {"status": "complete", "pid": os.getpid(), "epoch": state.epoch,
             "distributed_world_size": distributed.world_size}))

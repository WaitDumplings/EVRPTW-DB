...
VALIDATION_EVERY_EPOCHS="${VALIDATION_EVERY_EPOCHS:-250}"
...
case "$SCALE" in
  ...
  Cus100)
    ...
    ;;
  *)
    echo "This launcher supports only Cus50 or Cus100: $SCALE" >&2
    exit 2
    ;;
esac
...
if (( VALIDATION_EVERY_EPOCHS <= 0 || EARLY_STOP_PATIENCE <= 0 )); then
  echo "Validation interval and early-stop patience must be positive" >&2
  exit 2
fi

# validation_epochs() includes the minimum and maximum endpoints even when an
# interval does not land on them.  Keep the command-line checkpoint count in
# exact agreement with that schedule.
...
CUDA_VISIBLE_DEVICES="$GPU" python -m \
  ...
  --post-minimum-validation-every-epochs "$VALIDATION_EVERY_EPOCHS" \
  --validation-checkpoints "$VALIDATION_CHECKPOINTS" \
  --protocol-id rrnco_ev_single_seed_10k_early_stop_v1 \

...
if __package__:
    from .common import CONFIG, OUTPUT, REPO, TRAIN_INDEX, VAL_INDEX, digest, load_config, resolve_road_root, timestamp, write_json
else:
    from common import CONFIG, OUTPUT, REPO, TRAIN_INDEX, VAL_INDEX, digest, load_config, resolve_road_root, timestamp, write_json
...
def inspect_data(root, config, *, load_samples=True):
    ...
    return {"root": str(root), **hashes, "train_views": len(train), "validation_views": len(val),

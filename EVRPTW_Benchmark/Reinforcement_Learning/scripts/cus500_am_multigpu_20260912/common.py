"""Portable paths and atomic records for this independent Cus500 experiment."""
...
OUTPUT = REPO / "EVRPTW_Benchmark/results/cus500_am_multigpu_20260912"
...
def parse_gpus(value):
    ...
    if not fields or any(not field.strip().isdigit() for field in fields):
        raise ValueError("CUS500_GPUS must be comma-separated physical GPU indices, e.g. 0,1,2")
    result = [int(field.strip()) for field in fields]
    if len(result) != len(set(result)) or len(result) < 2:
        raise ValueError("This deployment requires at least two distinct GPU indices")
    return result
...
def load_config(path=CONFIG, *, gpus="0,1,2", batch=None, accumulation=None):
    config = json.loads(Path(path).read_text())
    if config.get("schema") != "cus500_am_multigpu_config_v1" or config.get("scale") != "Cus500":
        raise ValueError("Not a Cus500 AM deployment configuration")

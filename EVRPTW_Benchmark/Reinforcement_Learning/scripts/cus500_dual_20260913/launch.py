#!/usr/bin/env python3
"""Launch one Road Cus500 RRNCO or DRL-TS model across exactly two GPUs."""
...
if __package__:
    from .common import CONFIGS, MODELS, HERE, OUTPUT, REPO, TRAIN_INDEX, VAL_INDEX, load_config, resolve_road_root, source_snapshot, timestamp, write_json
    ...
    try:
        for gpu in sorted(selected, key=lambda row: row["uuid"]):
            ...
            try:
                ...
                raise RuntimeError(f"GPU {gpu['index']} ({identity}) is reserved by another Cus500 dual launcher") from None
        return descriptors
    except Exception:
...
def main():
    ...
    parser.add_argument("--gpus", default=os.environ.get("CUS500_GPUS", "0,1"))
    ...
    parser.add_argument("--instance-cache-size", type=int, default=os.environ.get("CUS500_INSTANCE_CACHE_SIZE"))

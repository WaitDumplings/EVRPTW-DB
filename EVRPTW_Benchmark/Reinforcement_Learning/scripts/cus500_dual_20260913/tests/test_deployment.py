...
from EVRPTW_Benchmark.Reinforcement_Learning.scripts.cus500_dual_20260913.common import (
    CONFIGS, TRAIN_INDEX, VAL_INDEX, load_config, parse_gpus, resolve_road_root,
)
from EVRPTW_Benchmark.Reinforcement_Learning.scripts.cus500_dual_20260913.launch import (
...
@pytest.mark.parametrize("value", ["", "0", "0,0", "-1,2", "0,,1", "0,banana", "0,1,2"])
def test_gpu_list_rejects_invalid(value):
    with pytest.raises(ValueError):
        parse_gpus(value)


@pytest.mark.parametrize("model", ["rrnco", "drl_ts"])
def test_two_card_global_budgets(model):
    config = load_config(model=model, gpus="1,2", batch=8, accumulation=2)
...
def test_config_rejects_wrong_validation_cap(tmp_path):
    config = json.loads((CONFIGS / "rrnco.json").read_text())
...
def test_stream_preparation_reuses_exact_global_budget_and_rejects_corrupt_content(tmp_path):
    ...
    from EVRPTW_Benchmark.Reinforcement_Learning.scripts.cus500_dual_20260913.common import digest
    from EVRPTW_Benchmark.Reinforcement_Learning.scripts.cus500_dual_20260913.prepare import prepare_stream
...
@pytest.mark.parametrize("model,module", [("rrnco", "RRNCO_EVRPTW"), ("drl_ts", "DRL_TS")])
def test_native_model_recipe_and_cache(model, module, tmp_path):
    cfg = load_config(model=model)
    command = build_command(cfg, tmp_path, tmp_path, {"path": "/stream", "contract": {"sha256": "abc"}})
    assert f"EVRPTW_Benchmark.Reinforcement_Learning.{module}.distributed_train" in command
    assert command[command.index("--instance-cache-size") + 1] == "256"
    if model == "rrnco":
        for key, expected in (("--graph-mode", "full"), ("--aft-mode", "stable"),
                              ("--distance-sampling", "nearest"), ("--relation-temperature", "5"),
                              ("--reinforce-baseline", "leave_one_out"),
                              ("--activation-checkpoint-stride", "1")):
            assert command[command.index(key) + 1] == expected
        assert "--checkpoint-bias" in command
        assert "--soft-stage-end-epoch" not in command
    else:
        for key, expected in (("--activation-checkpoint-stride", "1"), ("--batches-per-epoch", "250"),
                              ("--soft-stage-end-epoch", "2500")):
            assert command[command.index(key) + 1] == expected
        assert command[command.index("--method-auxiliary-profile") + 1].endswith("drl_ts_soft_auxiliary_v1.json")
        assert "--graph-mode" not in command


def test_model_configs_cannot_be_confused():
    with pytest.raises(ValueError, match="rrnco"):
        load_config(CONFIGS / "drl_ts.json", model="rrnco")
...
def test_model_launcher_output_paths_are_isolated(tmp_path):
    rr, rr_path = lock_output(tmp_path, load_config(model="rrnco"))
    drl, drl_path = lock_output(tmp_path, load_config(model="drl_ts"))
    try:
        assert rr_path != drl_path
        assert rr_path.parts[-2:] == ("2080ti_4_1", "rrnco")
        assert drl_path.parts[-2:] == ("2080ti_4_2", "drl_ts")
    finally:
        os.close(rr)
        os.close(drl)
...
def test_cuda_unavailable_does_not_prevent_shell_status(tmp_path):
    import subprocess
    import sys
    script = CONFIGS.parent / "2080ti_4_1/full.sh"
...
def test_worker_requires_successful_formal_result_and_best(tmp_path, monkeypatch, result_status, has_best, returncode, expected):
    import sys
    import time
    from EVRPTW_Benchmark.Reinforcement_Learning.scripts.cus500_dual_20260913 import launch

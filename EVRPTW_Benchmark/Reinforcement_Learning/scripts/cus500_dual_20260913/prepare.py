#!/usr/bin/env python3
"""Verify existing Road Cus500 data and freeze one global training-ID stream."""
...
if __package__:
    from .common import CONFIGS, MODELS, OUTPUT, REPO, TRAIN_INDEX, VAL_INDEX, digest, load_config, resolve_road_root, timestamp, write_json
else:
    from common import CONFIGS, MODELS, OUTPUT, REPO, TRAIN_INDEX, VAL_INDEX, digest, load_config, resolve_road_root, timestamp, write_json
...
def inspect_data(root, config, *, load_samples=True):
    ...
    train = index.loc[(index.customer_count == 500) & (index.split_id == "train") & (index.track_id == "train")]
    val_all = pd.read_parquet(root / VAL_INDEX)
    val = val_all.loc[(val_all.customer_count == 500) & (val_all.split_id == "val") & (val_all.track_id == "validation")]
    if len(train) != 10000 or len(val) != 500:
        raise ValueError(f"Expected 10000 Cus500 train and 500 val, found {len(train)} / {len(val)}")
    ...
    if set(train.terminal_count) != {551} or set(val.terminal_count) != {551}:
        raise ValueError("Expected 551 nodes in every Cus500 view")
    ...
    scale = reward.for_scale("Cus500", objective=objective)
    if load_samples:
        ...
        for split, path in (("train", TRAIN_INDEX), ("val", VAL_INDEX)):
            pool = Stage2TaskPool(root / path, scale="Cus500", split_ids=split, representation="G")
            instance = pool.instance(pool.tasks[0])
            if instance.distance_matrix_km.shape != (551, 551):
...
def prepare_stream(root, artifact_root, config, *, audit=None):
    ...
    lock = directory / f".road_cus500_seed{config['seed']}_n{config['sample_count']}.lock"
...
def _prepare_stream_unlocked(root, artifact_root, config, *, audit=None):
    ...
    root = Path(root).resolve()
    path = Path(artifact_root).resolve() / "streams" / f"road_cus500_seed{config['seed']}_n{config['sample_count']}.parquet"
    sidecar = path.with_suffix(path.suffix + ".manifest.json")
    if path.exists() != sidecar.exists():
        raise RuntimeError(f"Incomplete stream artifact; inspect {path} and {sidecar}")
    if not path.exists():
        index = pd.read_parquet(root / TRAIN_INDEX)
        frame, metadata = build_training_stream(index, scale="Cus500", seed=config["seed"], sample_count=config["sample_count"])
    ...
    result = {"schema": "cus500_global_stream_preparation_v1", "time": timestamp(), "data": audit,

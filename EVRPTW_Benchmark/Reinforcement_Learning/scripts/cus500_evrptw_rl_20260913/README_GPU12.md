# EVRPTW-RL Road Cus500：GPU1/2 双卡重训
...
bash EVRPTW_Benchmark/Reinforcement_Learning/scripts/cus500_evrptw_rl_20260913/evrptw_rl_gpu12.sh
```

GPU1 和 GPU2 通过 torchrun/NCCL 共同训练一个 Road Cus500 模型，每张卡处理不同实例，梯度同步后进行同一次全局更新。入口固定这两张物理卡，检查空闲后直接后台训练，无 watcher、无启动前校准。
...
配置由 `config_gpu12_mean.json` 保存。默认每 epoch 暴露 48×500=24,000 个客户，最大训练流 480,000 个实例条目。启动时自动从已有 Road train index 生成确定性训练流，不生成或重划分原始数据，不读取 test。
...
默认独立输出：
`EVRPTW_Benchmark/results/cus500_evrptw_rl_mean_gpu12_20260914`。
...
```bash
bash EVRPTW_Benchmark/Reinforcement_Learning/scripts/cus500_evrptw_rl_20260913/evrptw_rl_gpu12.sh --mode status
```

自定义输出可用 `--output-root /path/to/new/run` 或 `CUS500_EVRPTW_GPU12_OUTPUT_ROOT`，查看进度时使用同一位置。若要开始另一轮从头训练，可指定带时间的输出目录：

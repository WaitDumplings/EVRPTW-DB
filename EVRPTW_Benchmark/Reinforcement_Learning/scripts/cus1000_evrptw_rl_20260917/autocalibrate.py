#!/usr/bin/env python3
"""Verify the fixed Cus1000 EVRPTW-RL batch on free GPU 0/1/2/3.

Run explicitly before formal training. Probes are independent scratch runs; the final
...
def require_source(expected):
    if expected and source_snapshot()["source_sha256"] != expected:
        raise RuntimeError("Source changed during GPU calibration")
...
def probe_config(config, batch, *, confirmation=False):
    ...
    result.update(physical_batch_size=int(batch), gradient_accumulation_steps=1,
                  ...
                  protocol_id="cus1000_evrptw_rl_disposable_memory_probe_v1",
    ...
    result["world_size"] = 4
    result["effective_batch_size"] = int(batch) * 4
    result["sample_count"] = result["effective_batch_size"] * epochs
    result["customer_exposure_budget"] = result["sample_count"] * 1000
    return result
...
def run_probe(config, road_root, directory, selected, lock_fds, *, audit, batch,
    ...
    result = {"status": status, "batch_per_gpu": batch, "global_batch": batch * 4,
...
def calibrate(config, road_root, output_root, *, expected_source_sha256=None):
    """Confirm the configured batch; never change the user's fixed batch size."""
    ...
    try:
        ...
        batch = int(config["physical_batch_size"])
        session = root / f"fixed_batch_{batch}_{time.time_ns()}"
        session.mkdir()
        write_json(session / "preflight.json", audit)
        directory = session / "confirmation"
        write_json(root / "status.json", {"status": "calibrating", "time": timestamp(),
                   "batch_per_gpu": batch, "phase": "confirmation", "probe_output": str(directory),
                   "selection_strategy": "fixed_configuration_batch"})
        confirmation = run_probe(config, road_root, directory, selected, locks, audit=audit["data"],
                                 batch=batch, confirmation=True)
        peaks = confirmation.get("peak_process_gib", {})
        ceiling = float(config["target_process_memory_gib"][1])
        if (confirmation["status"] != "passed" or set(peaks) != {"0", "1", "2", "3"}
                or any(not math.isfinite(v) or not 0 < v <= ceiling for v in peaks.values())):
            raise RuntimeError(f"Fixed batch {batch} did not pass confirmation: {confirmation}")
        require_source(expected_source_sha256)
        final = copy.deepcopy(config)
        final.update(calibration_status="passed_local_four_gpu_profile")
        ...
        report = {"status": "passed", "time": timestamp(), "selected_batch_per_gpu": batch,
                  "global_batch": final["effective_batch_size"],
                  "target_process_memory_gib": final["target_process_memory_gib"],
                  "within_target": all(float(final["target_process_memory_gib"][0]) <= v <= ceiling
                                       for v in peaks.values()),
                  "confirmation": confirmation, "selection_strategy": "fixed_configuration_batch",
                  "formal_config": str(final_path), "stream_contract": stream["contract"],
                  "source_sha256": expected_source_sha256,
                  "scope": "GPU engineering confirmation, not convergence or benchmark performance"}

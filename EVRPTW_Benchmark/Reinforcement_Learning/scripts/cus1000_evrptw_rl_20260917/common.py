"""Portable paths and atomic records for the independent Cus1000 EVRPTW-RL experiment."""
...
MODELS = ("evrptw_rl",)
OUTPUT = REPO / "EVRPTW_Benchmark/results/cus1000_evrptw_rl_20260917"
...
def parse_gpus(value, *, fixed_gpus=(0, 1, 2, 3)):
    fields = str(value).split(",")
    if not fields or any(not field.strip().isdigit() for field in fields):
        raise ValueError("CUS1000_GPUS must be comma-separated physical GPU indices, e.g. 0,1,2,3")
    result = [int(field.strip()) for field in fields]
    if len(result) != len(set(result)) or len(result) != 4:
        raise ValueError("This deployment requires exactly four distinct GPU indices")
...
def resolve_road_root(value=None, repo=REPO):
    repo = Path(repo)
    requested = value or os.environ.get("CUS1000_ROAD_ROOT") or os.environ.get("CUS500_ROAD_ROOT") or os.environ.get("CUS100_ROAD_ROOT")
    if requested:
        ...
        if not (root / TRAIN_INDEX).is_file() or not (root / VAL_INDEX).is_file():
            raise FileNotFoundError(f"Road train/val indexes missing under {root}; set CUS1000_ROAD_ROOT")
        return root
    ...
    for root in candidates:
        if (root / TRAIN_INDEX).is_file() and (root / VAL_INDEX).is_file():
            return root.resolve()
    raise FileNotFoundError("Road release is unavailable; set CUS1000_ROAD_ROOT to the extracted full release")


def load_config(path=None, *, model="evrptw_rl", gpus="0,1,2,3", batch=None, accumulation=None, cache_size=None):
    if model not in MODELS:
        raise ValueError(f"Unsupported model: {model}")
    path = Path(path) if path is not None else CONFIG
    config = json.loads(path.read_text())
    if (config.get("schema") != "cus1000_evrptw_rl_config_v1" or config.get("scale") != "Cus1000"
            or config.get("model") != model):
        raise ValueError(f"Not a Cus1000 {model} four-GPU deployment configuration")
    if config.get("fixed_gpus", [0, 1, 2, 3]) != [0, 1, 2, 3]:
        raise ValueError("This Cus1000 deployment is fixed to physical GPUs 0,1,2,3")
    ...
    config["customer_exposure_budget"] = config["sample_count"] * 1000
    return config
...
def source_snapshot(repo=REPO):
    ...
    return {"schema": "cus1000_actual_source_v1", "source_sha256": identity,

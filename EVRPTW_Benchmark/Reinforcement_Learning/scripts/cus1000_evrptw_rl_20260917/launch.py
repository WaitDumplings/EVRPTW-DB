#!/usr/bin/env python3
"""Launch Road Cus1000 EVRPTW-RL on physical GPUs 0, 1, 2, and 3."""
...
if __package__:
    from .common import CONFIG, MODELS, HERE, OUTPUT, REPO, TRAIN_INDEX, VAL_INDEX, load_config, resolve_road_root, source_snapshot, timestamp, write_json
...
def environment_report(config):
    ...
    module_path = REPO / (config["train_module"].replace(".", "/") + ".py")
    if not module_path.is_file():
        raise FileNotFoundError(f"Multi-GPU trainer missing; update to the Cus1000 branch: {module_path}")
    memory = {}
    for line in Path("/proc/meminfo").read_text().splitlines():
        key, value = line.split(":", 1)
        if key in {"MemTotal", "MemAvailable"}:
            memory[key + "_gib"] = int(value.split()[0]) / (1024 * 1024)
    return {"host_memory": memory, "instance_cache_size_per_rank": config["instance_cache_size"],
            "python": sys.executable, "python_version": sys.version, "conda_environment": os.environ.get("CONDA_DEFAULT_ENV"),
...
def build_command(config, root, run, stream, *, python=sys.executable, resume=False):
    ...
    values = {
        ...
        "scale": "Cus1000", "split-ids": "train", "track-ids": "train", "training-representation": "G",
...
def preflight(config, root, output, *, resume=False):
    ...
    if resume:
        ...
        raise FileExistsError(f"Output already contains a run: {run}; use explicit --resume or a new CUS1000_OUTPUT_ROOT")
    target = Path(output).resolve()
    while not target.exists():
        target = target.parent
    if not os.access(target, os.W_OK | os.X_OK):
        raise PermissionError(f"Output parent is not writable: {target}")
    return {"schema": "cus1000_multigpu_preflight_v1", "time": timestamp(), "host": socket.gethostname(),
...
def lock_output(output, config):
    directory = Path(output) / "launchers" / config["server"] / config["model"]
    ...
    try:
        ...
        os.close(fd)
        raise RuntimeError("A Cus1000 launcher is already running in this output directory") from None
    return fd, directory


def lock_gpus(selected, *, lock_root=None):
    """Prevent this deployment from racing another model/output on the same card."""
    # Keep the shared lock namespace used by Cus500 deployments for cross-experiment exclusion.
    directory = Path(lock_root) if lock_root else Path(tempfile.gettempdir()) / f"evrptw-cus500-dual-gpu-locks-{os.getuid()}"
    directory.mkdir(parents=True, exist_ok=True, mode=0o700)
    descriptors = []
    try:
        for gpu in sorted(selected, key=lambda row: row["uuid"]):
            identity = gpu["uuid"]
            if not identity or any(ch not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-" for ch in identity):
                raise ValueError(f"Unexpected GPU UUID: {identity!r}")
            fd = os.open(directory / (identity + ".lock"), os.O_CREAT | os.O_RDWR, 0o600)
            descriptors.append(fd)
            try:
                fcntl.flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
            except BlockingIOError:
                raise RuntimeError(f"GPU {gpu['index']} ({identity}) is reserved by another EVRPTW launcher") from None
        return descriptors
    except Exception:
...
def worker(request_path, lock_fds):
    # Inherited output and per-GPU flocks cover startup, torchrun, and cleanup.
    for fd in lock_fds:
        os.fstat(fd)
    ...
    state = {"schema": "cus1000_multigpu_status_v1", "status": "starting", "time": timestamp(),
...
def start(args, config, root, output):
    lock_fd, launcher = lock_output(output, config)
    lock_fds = [lock_fd]
    handed_off = False
    try:
        audit = preflight(config, root, output, resume=args.resume)
        lock_fds.extend(lock_gpus(audit["gpus"]))
        stream = prepare_stream(root, Path(output) / "artifacts", config, audit=audit["data"])
        source = source_snapshot()
        request = {"schema": "cus1000_multigpu_launch_request_v1", "time": timestamp(),
        ...
        if args.foreground:
            handed_off = True
            return worker(request_path, lock_fds)
        with (launcher / "launcher.log").open("ab") as log:
            child = subprocess.Popen([sys.executable, str(HERE / "launch.py"), "--mode", "worker", "--request", str(request_path),
                                      "--lock-fds", ",".join(str(fd) for fd in lock_fds)], cwd=REPO, stdout=log, stderr=log,
...
def last_jsonl(path):
    ...
    if not path.is_file():
        return None
    # Training appends concurrently. Ignore an unfinished last row.
...
def read_status(output, config):
    output = Path(output)
    path = output / "launchers" / config["server"] / config["model"] / "status.json"
    if not path.is_file():
        return {"status": "not_started", "status_file": str(path), "model": config["model"]}
    state = json.loads(path.read_text())
    run = Path(state["output_dir"])
    state["status_file"] = str(path)
    state["latest_train"] = last_jsonl(run / "logical_epoch_history.jsonl")
    state["latest_validation"] = last_jsonl(run / "validation_history.jsonl")
    for field, name in (("best_validation", "validation_summary.json"), ("progress", "progress.json")):
        source = run / name
        state[field] = json.loads(source.read_text()) if source.is_file() else None
    if state.get("status") in {"starting", "running"}:
        try:
            os.kill(int(state["launcher_pid"]), 0)
            state["launcher_alive"] = True
        except (ProcessLookupError, KeyError, ValueError):
            state["launcher_alive"] = False
            state["status"] = "launcher_missing_check_training_processes"
        except PermissionError:
            state["launcher_alive"] = None
    return state
...
def main():
    ...
    parser.add_argument("--lock-fds", help=argparse.SUPPRESS)
    args = parser.parse_args()
    if args.mode == "worker":
        if args.request is None or args.lock_fds is None:
            parser.error("worker is internal and requires an inherited launch lock")
        return worker(args.request, [int(fd) for fd in args.lock_fds.split(",")])
    if args.model is None:
        parser.error("--model is required (or use a per-server full.sh)")
    ...
    if args.mode == "status":
        print(json.dumps(read_status(output, config), indent=2))
        return 0

...
CPU验证78项（58原有+11部署+4固定batch+1入口锁定+4正式配置）与真实四卡短训分别记录。详见[验证报告](VALIDATION_REPORT.md)及[精简实测数据](measured_profile.json)。校准不构成收敛或算法性能结论，未使用测试集。

## 已启动旧版并遇到早停参数错误

初版`51ba4a2`把早停起点设为2000，触发训练器`early-stop start must precede maximum training epochs`检查；失败发生在第一次训练更新之前。修复后保持2000次更新，显式关闭早停，并在后台启动前校验组合。

已有部署worktree可更新后使用新输出目录启动；原失败日志保留，不能对这次未生成checkpoint的失败使用`--resume`：

```bash
cd /data/Maojie/ICLR/cus1000-evrptw-rl-4gpu-deploy
git fetch origin
git merge --ff-only origin/cus1000-evrptw-rl-4gpu-20260917
conda activate maojie
export CUS1000_OUTPUT_ROOT="$PWD/EVRPTW_Benchmark/results/cus1000_evrptw_rl_20260917_fixed"
bash EVRPTW_Benchmark/Reinforcement_Learning/scripts/cus1000_evrptw_rl_20260917/2080ti_4_2/full.sh
```

后续查询状态和恢复也需设置同一个`CUS1000_OUTPUT_ROOT`。修复后已用完整正式配置通过实际`full.sh`四卡首更新测试，随后主动停止本机独立测试；该验证没有缩小2000/2000轮配置。
